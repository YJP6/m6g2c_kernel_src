...
exit:
	if (unlikely(err)) {
		...
		if ((err == BCME_EPERM) && cfg->scan_suppressed)
			...
			// l_patches send disassoc to firmware
			if (cnt >= 4) {
...
// l_patches let it scan in SoftAP mode
//		return 0;
	}
...
#endif
{
	...
	if (wl_get_mode_by_netdev(cfg, dev) == WL_MODE_AP) {
		...
		// l_patches limit the max. rssi to -2 or the bss will be filtered out in android OS
...
#if defined(BSSCACHE)
	if (cfg->p2p_disconnected > 0) {
		// l_patches Fix for wrong group_capab (timing issue)
		wl_delete_disconnected_bss_cache(&g_bss_cache_ctrl, (u8*)&cfg->disconnected_bssid);
#if defined(RSSIAVG)
		wl_delete_disconnected_rssi_cache(&g_rssi_cache_ctrl, (u8*)&cfg->disconnected_bssid);
#endif
...
// l_patches fix for roaming issue
#if 0
...
static void wl_scan_timeout(unsigned long data)
{
	...
	// l_patches workaround to fix out of memory in firmware
...
wl_cfg80211_set_auto_channel_scan_state(struct net_device *ndev)
{
	...
	if (ret < 0) {
		...
		ret = BCME_OK; // l_patches fix escan_complete error
		goto done;
	}
...
wl_cfg80211_get_best_channels(struct net_device *dev, char* cmd, int total_len)
{
	...
	if (band_cur != WLC_BAND_2G) {
		// l_patches fix for some chipsets only return 2.4GHz channel (4330b2/43341b0/4339a0)

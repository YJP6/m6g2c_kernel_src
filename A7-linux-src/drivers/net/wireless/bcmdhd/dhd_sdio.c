...
dhdsdio_write_vars(dhd_bus_t *bus)
{
	...
	// l_patches fix for nvram failed to download
...
dhd_bus_init(dhd_pub_t *dhdp, bool enforce_mutex)
{
	...
	if (bus->sih->chip == BCM43362_CHIP_ID) {
		...
		OSL_DELAY(100000); // l_patches delay for 43362
	}
		...
		for (idx = 0; idx < DHD_MAX_IFS; idx++) {
			if (list_head[idx]) {
				...
				if (cnt) {
					...
					/* l_patches fix for below error due to bussleep in watchdog after dhd_os_sdunlock here,
...
dhdsdio_isr(void *arg)
{
	...
	/* l_patches dpc should be scheded again if dpc_sched is TRUE or dhd_bus_txdata can
...
dhd_bus_watchdog(dhd_pub_t *dhdp)
{
	...
	// l_patches remove !SLPAUTO_ENAB(bus) to fix not able to polling if sr supported
...
#if defined(MULTIPLE_SUPPLICANT)
extern void wl_android_post_init(void); //l_patches fix critical section in dhd_open and dhdsdio_probe
#endif
...
#if 0 // l_patches fix for WPA/WPA2 4-way handshake fail in hostapd
...
#if defined(MULTIPLE_SUPPLICANT)
	wl_android_post_init(); // l_patches fix critical section in dhd_open and dhdsdio_probe
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 25))
...
dhd_set_path_params(struct dhd_bus *bus)
{
	...
	if (firmware_path[0] == '\0') {
		...
		strncpy(bus->fw_path, firmware_path, MOD_PARAM_PATHLEN-1);
	}
	if (nvram_path[0] == '\0') {
		dhd_conf_set_nv_name_by_chip(bus->dhd, bus->nv_path);
	} else {
		strncpy(bus->nv_path, nvram_path, MOD_PARAM_PATHLEN-1);
	}
	if (clm_path[0] == '\0') {
		dhd_conf_set_clm_name_by_chip(bus->dhd, bus->dhd->clm_path);
	} else {
		strncpy(bus->dhd->clm_path, clm_path, MOD_PARAM_PATHLEN-1);
	}
...
#ifdef SUPPORT_P2P_GO_PS
	// l_patches fix for suspend issue
...
dhdsdio_download_code_file(struct dhd_bus *bus, char *pfw_path)
{
	...
	uint8 *memptr_tmp = NULL; // l_patches check downloaded firmware is correct
	...
	while ((len = dhd_os_get_image_block((char*)memptr, MEMBLOCK, image))) {
		// l_patches fix for firmware failed to download
		if (bus->dhd->conf->chip == BCM43340_CHIP_ID ||
				bus->dhd->conf->chip == BCM43341_CHIP_ID) {
			if (len % 64 != 0) {
...
#if defined(HW_OOB) || defined(FORCE_WOWLAN)
			dhd_conf_set_hw_oob_intr(bus->sdh, bus->sih->chip); // l_patches fix for OOB initial issue
#endif

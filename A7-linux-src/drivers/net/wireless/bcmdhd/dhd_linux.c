...
// l_patches fix building error if ARP_OFFLOAD_SUPPORT removed
...
#define CUSTOM_INTR_WIDTH 100
	...
	if (dhd->up) {
		if (value && dhd->in_suspend) {
			...
			/* l_patches Reject in early suspend */
...
dhd_set_mac_addr_handler(void *handle, void *event_info, u8 event)
{
	...
	// l_patches fix for not able to set mac when wlan0 is down
...
dhd_os_wlfc_block(dhd_pub_t *pub)
{
	...
	ASSERT(di != NULL);
	/* l_patches don't do spin lock if proptx not enabled */
	if (disable_proptx)
		return 1;
...
#endif
	/* l_patches Micky add to ajust the 802.1X priority */
...
#if 0
			// l_patches remove this to prevent dev_close() get stuck in dhd_hang_process
...
#ifdef CONFIG_HAS_WAKELOCK
	// l_patches can not destroy wl_wifi when wlan down, it will happen null pointer in dhd_ioctl_entry
...
//	setbit(eventmask, WLC_E_SCAN_COMPLETE); //l_patches remove redundant event
#ifdef DHD_DEBUG
	...
	// l_patches set bus:txglom after dhd_txglom_enable since it's possible changed in dhd_conf_set_txglom_params
	...
	if (ret == 0){
		...
		disable_proptx = 1;
		wlfc_enable = FALSE;
		/* l_patches we should set ampdu_hostreorder=0 when disalbe_proptx=1 */
		hostreorder = 0;
	}
	...
	if (wlfc_enable) {
		dhd_wlfc_init(dhd);
		/* l_patches enable ampdu_hostreorder if tlv enabled */
...
#endif /* DISABLE_11N */
#else
	/* l_patches disable ampdu_hostreorder if PROP_TXSTATUS not defined */
...
dhd_os_tcpackunlock(dhd_pub_t *pub, unsigned long flags)
{
	...
	if (dhd) {
#ifdef BCMSDIO
		spin_unlock_bh(&dhd->tcpack_lock); //l_patches
#else
...
static void dhd_hang_process(void *dhd_info, void *event_info, u8 event)
{
	...
	if (dev) {
		//l_patches let wlan0 down when hang happened
		rtnl_lock();
		dev_close(dev);
		rtnl_unlock();
#if defined(WL_WIRELESS_EXT)

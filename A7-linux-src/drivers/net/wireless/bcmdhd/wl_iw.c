...
wl_iw_event(struct net_device *dev, wl_event_msg_t *e, void* data)
{
	...
	case WLC_E_SCAN_COMPLETE:
		...
		// l_patches fix "wlan0: (WE) : Wireless Event too big (65306)"
		memset(&wrqu, 0, sizeof(wrqu));
...
int wl_iw_get_wireless_stats(struct net_device *dev, struct iw_statistics *wstats)
{
	...
	if ((res = wldev_ioctl(dev, WLC_GET_ASSOCLIST, assoc_maclist, sizeof(mac_buf), false)))
		goto done;
	WL_TRACE(("wl_iw_get_wireless_stats assoclist count=%d\n *****", assoc_maclist->count));
	wstats->status = (assoc_maclist->count > 0)? 1 : 0;
